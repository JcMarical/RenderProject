Shader "Tutorial/CloudMarching"
{
    ...
    /*
            float SampleNoiseDensity(float3 worldPos) {
                ...
                float4 heightCurveA = tex2Dlod(_HeightCurveA, heightCurveUV);
                float4 heightCurveB = tex2Dlod(_HeightCurveB, heightCurveUV);

                float4 noise2DbUV = float4(worldPos.xz, 0, 0);
                noise2DbUV.xy *= _Noise2DbTile.xy * _Noise2DbTile.w;
                float4 noise2Db = tex2Dlod(_Noise2Db, noise2DbUV);

                float4 heightCurve = lerp(heightCurveA, heightCurveB, noise2Db);
...
*/
        ...
        float stepCloudDensity = 0; //SampleNoiseDensity(densityStepPos) * _DensityScale;
            ...
            depth += 0; //SampleNoiseDensity(lightMarchPos);
        }

                    depth /= _LightIteration;

                    float4 lightInAttenuation = tex2D(_LightAttenuationCurve, float4(depth, 0, 0, 0)); 
                    float4 lightOutAttenuation = tex2Dlod(_LightAttenuationCurve, float4(cloudDensity, 0, 0, 0)); 
