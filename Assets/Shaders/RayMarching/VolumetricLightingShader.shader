Shader "Unlit/VolumetricLightingShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            TEXTURE2D_X_FLOAT(_CameraDepthTexture); SAMPLER(sampler_CameraDepthTexture);//采样相机深度纹理
            TEXTURE2D(_CameraOpaqueTexture); SAMPLER(sampler_CameraOpaqueTexture);//采样相机不透明纹理
            ...
            {
                half2 screenPos = i.screenPos.xy / i.screenPos.w;//屏幕坐标等于xy除以w分量
                //rebuild world position according to depth
                float depth = SAMPLE_TEXTURE2D_X(_CameraDepthTexture,sampler_CameraDepthTexture, screenPos).r;
                depth = Linear01Depth(depth, _ZBufferParams);//获取深度图
                ...
                float3 startPos = i.worldPos;//像素点的世界坐标
